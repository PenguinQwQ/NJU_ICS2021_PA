...
static uintptr_t loader(PCB *pcb, const char *filename) {
  ...
  //    i * headers_entry_size + program_header_offset, sizeof(elf_header));
    fs_lseek(fd, i * headers_entry_size + program_header_offset, SEEK_SET);
   ...
   //   read(fd, phys_addr + (virt_addr & 0xfff), section_entry.p_offset, section_entry.p_filesz);
      ...
      memset(phys_addr + (virt_addr & 0xfff) + section_entry.p_filesz, 0, section_entry.p_memsz - section_entry.p_filesz);
